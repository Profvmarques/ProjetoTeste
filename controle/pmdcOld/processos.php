...
function controle_processos($acao) {
	switch ($acao) {
		case 'incluir':
			...
			require_once('../../classe/faetec/modalidades.php');
			...
			require('../../classe/faetec/assunto.php');
			$assunto = new assunto();
			...
			if ($_POST['incluir'] == 1) {
				...
				//chama a fun��o de inclus�o da classe processos
		...
		case 'visualizar':
			...
			require_once('../../classe/faetec/historico.php');
			...
			//if (($_POST['pesquisar'] != 1 && $_GET['p'] == '') || ($_POST['filtrar'] == 1 || $_GET['f'] == 1)) {
				//chama a fun��o de listagem da classe processos
				if ($_GET['inicio'] == '') {
			...
			//pesquisa
				...
				//chama a fun��o de pesquisa da classe processos
				if ($_GET['inicio'] == '') {
			...
			//numero total de registros para pagina��o
			$num = $processos -> num;
			
			//chama a fun��o de listagem da classe entidades
			$lista_entidades = $entidades -> listar($_SESSION['entidade']);

			//chama a fun��o de listagem da classe status
			$lista_status = $status -> listar();
			
			//chama a fun��o de listagem da classe tipo
			$lista_tipo = $tipos -> listar();
			
			//chama a fun��o de listagem da classe assunto
			$lista_assunto = $assunto -> listar();
			
			//chama a fun��o de listagem da classe deficiencia
			$lista_deficiencias = $deficiencia -> listar();
			
			//chama a fun��o de listagem da classe modalidades
			$lista_modalidades = $modalidades -> listar();
		break;
		...
		case 'bloco':
			...
			//chama a fun��o de listagem da classe modalidades (filtro)
			$lista_modalidades = $modalidades -> listar();
			
			//chama a fun��o de listagem da classe deficiencia (filtro)
			$lista_deficiencias = $deficiencia -> listar();
			
			//chama a fun��o de listagem da classe status (filtro)
			$lista_status = $status -> listar();
			
			//chama a fun��o de listagem da classe status (resposta)
			$lista_status_resposta = $status -> listar();
			
			//chama a fun��o de listagem da classe resposta_padrao
		...
		case 'detalhes':
			...
			//incluir visualiza��o no historico
			if (count($_POST) < 1) {
				$historico->incluir_visualizacao($_GET['id'],$_SESSION['usuario']);
			}
			
			//excluir manifesta�ao
			if ($_POST['excluir'] == 1) {
				$resultado = $processos -> excluir($_GET['id']);
				if ($resultado == 1) {	
					echo "<script>alert('Manifesta��o exclu�da');</script>";
			...
			if ($_POST['inibir']) {
				...
				if ($resultado == 1) {
					echo "<script>alert('Manifesta��o inibida');</script>";
					echo "<script>window.location='?pg=1'</script>";
					die;
				}
			}
			...
			if ($_POST['inibir_denuncia']) {
				$resultado = $processos -> inibir($_GET['id']);
				if ($resultado == 1) {
					echo "<script>alert('Den�ncia inibida');</script>";
			...
			if ($_POST['restaurar']) {
				...
				if ($resultado == 1) {	
					echo "<script>alert('Manifesta��o restaurada');</script>";
			...
			if ($_POST['btn_cidadao'] || $_POST['btn_vinculada']) {
				...
				try {
					...
					$destino = ($_POST['btn_cidadao'] != '') ? 'Cidad�o' : 'Vinculada';
					...
					if ($email == '') {
						...
						$mail->AltBody = 'Para ver esta mensagem, use um visualizador de e-mail compat�vel com HTML'; // optional - MsgHTML will create an alternate automatically
			...
			//chama a fun��o de listagem da classe modalidade de ensino
			$lista_modalidades = $modalidades -> listar();

			//chama a fun��o de listagem da classe resposta_padrao
			$lista_resposta = $resposta_padrao -> listar();
			
			//chama a fun��o para listar dados de um registro da classe processos
			$resultado = $processos -> detalhes($_GET['id']);
			
			//buscar permissoes do usuario
			$permissoes = $usuario -> permissoes($_SESSION['usuario']);
			
			//chama a fun��o para listar o historico de um registro da classe processos
		...
		case 'historico_publico':
			if ($_POST['enviar']) {
				...
				echo "<script>alert('Novo coment�rio registrado');</script>";
...
if ($_POST['historico_bloco']) {
	...
	foreach ($ids as $id) {
		$sql = "insert into ouv_historico 
						...
						'".date('Y-m-d H:i:s')."', 'E-mail enviado para ".$_POST['email']." (".utf8_decode($_POST['destino']).") com c�pia para: ".$_POST['copia']."',
						".$_SESSION['usuario'].", ".$_POST['publico'].")";
		$historico -> Incluir_email($sql);
	}
}
