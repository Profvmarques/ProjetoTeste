...
function Rect($x,$y,$w,$h,$style='',$orientacao='')
{
	//Draw a rectangle
	if($style=='F')
		$op='f';
...
function Sector($xc,  $yc,  $r,  $a,  $b,  $style='FD',  $cw=true,  $o=90){
	...
	if ($d < M_PI/2){
		$this->_Arc($xc+$r*cos($a)+$MyArc*cos(M_PI/2+$a),
				...
				$patual = $this->porcentagem[$this->ct];
				...
				$this->percents[$this->ct]['x'] = $mx;
				$this->percents[$this->ct]['y'] = $my;
				if ($patual > 0.00) {
					$this->percents[$this->ct]['txt'] = number_format($this->porcentagem[$this->ct],2)." %";
//					$this->percents[$this->ct]['txt'] .= $this->atual;
				}
	}else{
		$b = $a + $d/4;
		...
		$this->_Arc($xc+$r*cos($a)+$MyArc*cos(M_PI/2+$a),
				...
				if ($this->atual > 90 && $this->atual < 180 && $this->angulo >= 45) {
					$mx -= 90;
				}
				if ($this->atual < 180 && $this->atual > 15) {
					if ($patual <= 37.5) {
						$mx -= 20;
						$my += 30;
					}
					else {
						if ($patual <= 50 && $this->atual < 90) {
							$mx -= 35;
							$my += 30;
						}
						else {
...
function PieChart($w, $h, $data, $descricao, $format, $colors=null)
    {
        $this->SetFont('Courier', '', 10);
        $this->SetLegends($data, $format, $descricao);
		...
		//gambiarra pra fatia final
		if ($this->ct+1 == sizeof($data)) {
			$angleEnd = 360;
		}
...
function SetLegends($data, $format, $descricao,$barra='')
{
	$this->legends=array();
	$this->wLegend=0;
	...
	foreach($data as $l=>$val)
	{
		...
		if ($barra == '') {
			$this->legends[] = $descricao[$l].": ".$val." (".$p.")";
		}
		else {
			$this->legends[] = $descricao[$l];
		}
		if ($this->sum > 0) {
			$this->porcentagem[$l] = $val/$this->sum*100;
		}
		$this->wLegend=max($this->GetStringWidth($legend), $this->wLegend);
	}
}

function BarDiagram($w, $h, $data, $format, $descricao, $color=null, $inicio, $relatorio, $maxVal=0, $nbDiv=4){
	$this->SetFont('Courier', '', 10);
	$this->SetLegends($data, $format, $descricao, 1);
...
/*	$al = 100;//(int)($t * $unit);
	...
	for ($z=1;$z<11;$z++) {
		$this->Line(5,$pos,5,$pos - $div);
		$this->text(4,$pos-0.4 - $div +1,"-".$z."0%");
		$pos -= $div;
	}
*/	
	//margem superior
	...
	foreach($data as $val) {
		//if ($val > 0) {
			...
			$this->SetFillColor($color[$cor][0], $color[$cor][1], $color[$cor][2]);
			$cor ++;
			//$lval = largura;
			//$hval = altura;
			//$por = number_format($val/$this->sum*100,2);
			//$lval = (int)($val * $unit);
			//$lval = $por;
			$lval = $val;
			...
			$this->Rect($posicao_barra, $xval, $lval, $hval, 'DF',1);

	        //Legend
	        $this->SetXY(15+$lval,$xval+8);
			$this->SetFont('Arial','',$font);
			$this->Text($this->getx()+$pos_valor,$xval-1.5,$val);
			$this->SetXY(7,$xval-5);
			
			$this->drawTextBox($this->legends[$c], $tam_leg, 2,$alinhamento,'T',0);
			
			//$p = number_format($val/$this->sum*100,2);
			//$this->Cell(4.5,3,$val,0,0,'C');
			$c++;
			$n++;
