...
require_once('../../classe/faetec/diretoria.php');
...
$id_diretoria = $_POST['diretoria'];

if ($id_diretoria!="") {
	$tot = $diretoria -> contar($id_diretoria,$comp,$_POST['status'],$_POST['tipo_usuario'],$_POST['subitem']);

	$res = $diretoria -> detalhes($id_diretoria);
	$nome_diretoria = mysql_result($res,0,'desc_diretoria');
	...
	if ($tot==0) {
		...
		$pdf->Cell(0,10,'NÃO HÁ RESULTADOS PARA A DIRETORIA: '.$nome_diretoria,0,1,'C');
		...
		$pdf->SetFont('Arial','B',12);
		$pdf->Cell(0,10,'RELATÓRIOS DA OUVIDORIA - '.$nome_diretoria,0,1,'C');
		...
		for ($i=1;$i<=$total;$i++) {
			...
			$quantidade = $processos -> contar_dir($i,$id_diretoria,$comp,$_POST['status'],$_POST['tipo_usuario'],$_POST['subitem']);
		...
		$resultado = $processos -> buscar_tipos_dir($_POST['diretoria'],$comp,$_POST['status'],$_POST['tipo_usuario'],$_POST['subitem']);
		...
		$resultado = $processos -> processos_por_dia_dir($data_inicial, $data_final, $_POST['diretoria'], $_POST['status'], $_POST['tipo_usuario'],$_POST['subitem']);
		$pordia = array();
	...
	//Relatório para todas as diretorias
	$lista_diretoria = $diretoria -> listar(0);


		 

		
		$tot = $diretoria -> contar('',$comp,$_POST['status'],$_POST['tipo_usuario'],$_POST['subitem']);
		$res = $diretoria -> detalhes($id_diretoria);
		$nome_diretoria = mysql_result($res,0,'desc_diretoria');
		...
		if ($tot==0) {
			...
			for ($i=1;$i<=$total;$i++) {
				...
				$quantidade = $processos -> contar_dir($i,'',$comp,$_POST['status'],$_POST['tipo_usuario'],$_POST['subitem']);
			...
			$tipos -> inicializar();
			//buscar tipos de cada registro de um segmento da tabela ouvidoria
			$resultado = $processos -> buscar_tipos_dir($vazio,$comp,$_POST['status'],$_POST['tipo_usuario'],$_POST['subitem']);
			...
			$resultado = $processos -> processos_por_dia_dir($data_inicial, $data_final, $vazio, $_POST['status'], $_POST['tipo_usuario'],$_POST['subitem']);
			$pordia = array();
			while ($p = mysql_fetch_array($resultado)) {
				$pordia[$p['data']] = $p['total'];
				$datas[$p['data']] = $p['data'];
