...
$(document).ready(function(){
	...
	$("#resposta_padrao").change(function(){
		$.post('../../controle/faetec/processos.php',{id: $(this).val()},function(resposta){$("#resumo").val(resposta);});	
	});
	...
	$("#entidade_nova").change(function(){
		if (confirm("Tem certeza que deseja alterar a entidade?")) {
			$.post('../../controle/faetec/processos.php',{entidade_nova: $(this).val(),id_ouvidoria: <?php echo $_GET['id']; ?>});	
		}
		else {
	...
	$("#modalidade_nova").change(function(){
		if (confirm("Tem certeza que deseja alterar a modalidade?")) {
			$.post('../../controle/faetec/processos.php',{modalidade_nova: $(this).val(),id_ouvidoria: <?php echo $_GET['id']; ?>});	
		}
		else {
	...
	$("#inibir_dados").change(function() {
		if (confirm("Tem certeza que deseja alterar essa opção?")) {
			$.post('../../controle/faetec/processos.php',{inibir_dados_novo: $(this).val(),id_ouvidoria: <?php echo $_GET['id']; ?>},
			function(resposta) {
	...
	$("#tipo_novo").change(function(){
		if (confirm("Tem certeza que deseja alterar o tipo?")) {
			$.post('../../controle/faetec/processos.php',{tipo_novo: $(this).val(),id_ouvidoria: <?php echo $_GET['id']; ?>,
			denuncia:1});
		}
		else {
	...
	$("#assunto").change(function(){
		if (confirm("Tem certeza que deseja alterar o assunto?")) {
			$.post('../../controle/faetec/processos.php',{assunto_novo: $(this).val(),id_ouvidoria: <?php echo $_GET['id']; ?>});	
		}
		else {
...
<body>
<p>Visualização da den&uacute;ncia</p>
<table width="100%" border="0">
	<tr class='odd'>
		...
		<td width="10%" class="td">Data Limite:</td><td width="20%" class='textol'><?php echo $detalhes['data_fim'] ?></td>
	</tr>
	...
	<tr class="column1">
		...
		<td class="td">Defici&ecirc;ncia:</td><td class='textol'><?php echo ($detalhes['deficiencia'] == '') ? 'Nenhuma' : $detalhes['deficiencia']; ?></td>
	</tr>
	<tr class="odd">
	...
	<tr  class="column1">
		...
		<td colspan="5" class='textol'><?php echo $detalhes['email'] ?></td>
	</tr>
	<tr class="odd">
		<td class="td">Endere&ccedil;o:</td>
		<td colspan="5" class='textol'><?php echo $detalhes['endereco'] ?></td>
	</tr>
	<tr class="column1">
		<td class="td">Bairro:</td>
		<td class='textol'><?php echo $detalhes['bairro'] ?></td>
		<td class="td">Cidade:</td>
		<td colspan="3" class='textol'><?php echo $detalhes['cidade'] ?></td>
	</tr>
	<tr class="odd">
		<td class="td">Telefone:</td>
		<td class='textol'><?php echo $detalhes['telefone'] ?></td>
		<td class="td">Celular:</td>
		<td colspan="3" class='textol'><?php echo $detalhes['celular'] ?></td>
	</tr>
	<tr class="column1">
	...
	<tr class="odd">
		<td class="td">Primeira Manifesta&ccedil;&atilde;o:</td>
		<td class='textol'><?php echo ($detalhes['prim_rec'] != '') ? $detalhes['prim_rec'] : 'NÃO INFORMADO' ?></td>
		<td class="td">Protocolo(s) Anteriore(s):</td>
		<td colspan="5" class='textol'><?php echo $detalhes['protocolo_anterior'] ?></td>
	</tr>
	<tr class="column1">
		<td class="td">Den&uacute;ncia:</td><td colspan="5" class='textol'><?php echo nl2br($detalhes['comentario']) ?></td>
...
<input type='hidden' name='excluir' id='excluir' value = ''>
	<?php if ($permissoes['excluir']==1) { ?>
		<input type='submit' name='inibir_denuncia' id='inibir_denuncia' value = 'Inibir Den&uacute;ncia' onclick='return confirmar_inibicao();'>
