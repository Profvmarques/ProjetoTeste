...
$(document).ready(function(){
	...
	$("#resposta_padrao").change(function(){
		$.post('../../controle/pmdc/processos.php',{id: $(this).val()},function(resposta){$("#resumo").html(resposta);});	
	});
	...
	$("#entidade").change(function(){
		if (confirm("Tem certeza que deseja alterar a entidade?")) {
			$.post('../../controle/pmdc/processos.php',{entidade: $(this).val(),id_ouvidoria: <?php echo $_GET['id']; ?>});	
		}
		else {
...
<script>
	...
	function confirmar_restauracao() {
		if (!confirm('Tem certeza que deseja restaurar essa denúncia?')) {
			return false;
		}
	}
</script>
...
<input type='hidden' name='excluir' id='excluir' value = ''>
	<?php if ($permissoes['excluir']==1) { ?>
		<input type='submit' name='restaurar_denuncia' id='restaurar_denuncia' value = 'Restaurar Den&uacute;ncia' onclick='return confirmar_restauracao();'>
