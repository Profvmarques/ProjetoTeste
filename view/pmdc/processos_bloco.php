...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<link rel="stylesheet" type="text/css" href="../../css/pmdc/style_bloco.css" />
		...
		<script type="text/javascript">
			$().ready(function(){ 
				...
				$("#filtrar").click(function() {
					$("#processos").html("<img src='../../imagens/pmdc/loading2.gif'>");
					$.post('../../controle/pmdc/processos.php',{filtrar_bloco:1,status:$("#status").val(),tipo:$("#tipo").val(),
				...
				$("#alterar").click(function() {
					$("#acoes").html("Realizando altera&ccedil;&otilde;es... <img src='../../imagens/pmdc/loading2.gif'>");
					...
					if (ids.length > 0) {
						$.post('../../controle/pmdc/processos.php',{escolhido:1,ids:ids.join("|"),novo_status:$("#novo_status").val(),
				...
				$("#inibir").click(function() {
					...
					if (ids.length > 0) {
						$.post('../../controle/pmdc/processos.php',{inibir_blobo:1,ids:ids.join("|")},
						function (resposta) {
							alert('Alterações realizadas');
							window.location=window.location;
						});
				...
				$("#btn_cidadao_bloco").click(function() {
					...
					if (ids.length > 0) {
						$("#acoes").html("Enviando... <img src='../../imagens/pmdc/loading2.gif'>");
						$(this).attr('disabled',true);
						$.post('../../controle/pmdc/processos.php',{email_bloco:1,email:$("#email_cidadao").val(),destino:'Cidadão',
						nome:$("#nome_cidadao").val(),copia:$("#copia_cidadao").val(),resumo:$("#resumo").val(),publico:$("#publico").val()},
						function (resposta) {
							if (resposta == 1) {
								alert('Mensagem enviada');
								$.post('../../controle/pmdc/processos.php',{historico_bloco:1,ids:ids.join("|"),
				...
				$("#btn_vinculada").click(function() {
					...
					if (ids.length > 0) {
						$("#acoes").html("Enviando... <img src='../../imagens/pmdc/loading2.gif'>");
						$(this).attr('disabled',true);
						$.post('../../controle/pmdc/processos.php',{email_bloco:1,email:$("#email_vinculada").val(),destino:'Vinculada',ids:ids.join("|"),
				...
				$("#resposta_padrao").change(function(){
					$.post('../../controle/pmdc/processos.php',{id: $(this).val()},
					function(resposta){
						$("#resumo").val(resposta);
					});	
				});
